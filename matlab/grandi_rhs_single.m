function [values] = grandi_rhs_single(t, states, parameters, value_nr)
  ...
  Fjunc=parameters(1,:); Fjunc_CaL=parameters(2,:); cellLength=parameters(3,:);...
    ...
    gkp=parameters(16,:); pNaK=parameters(17,:); epi=parameters(18,:);...
  ...
  elseif value_nr == 11
      ...
      values = (-f + fss)./tauf;
  elseif value_nr == 12
      values = -0.0119.*f_Ca_Bj + 1.7.*(1 - f_Ca_Bj).*Ca_j;
  elseif value_nr == 13
      values= -0.0119.*f_Ca_Bsl + 1.7.*(1 - f_Ca_Bsl).*Ca_sl;
  elseif value_nr == 14
      kCaSR = MaxSR - (MaxSR - MinSR)./(1 + (ec50SR./Ca_sr).^2.5);
      koSRCa = koCa./kCaSR;
      kiSRCa = kiCa.*kCaSR;
      RI = 1 - Ry_Ri - Ry_Ro - Ry_Rr;
      values = kim.*RI + kom.*Ry_Ro - Ca_j.^2.*Ry_Rr.*koSRCa - Ca_j.*Ry_Rr.*kiSRCa;
  elseif value_nr == 15
      kCaSR = MaxSR - (MaxSR - MinSR)./(1 + (ec50SR./Ca_sr).^2.5);
      koSRCa = koCa./kCaSR;
      kiSRCa = kiCa.*kCaSR;
      values = kim.*Ry_Ri - kom.*Ry_Ro + Ca_j.^2.*Ry_Rr.*koSRCa - Ca_j.*Ry_Ro.*kiSRCa;
  elseif value_nr == 16
      kCaSR = MaxSR - (MaxSR - MinSR)./(1 + (ec50SR./Ca_sr).^2.5);
      koSRCa = koCa./kCaSR;
      kiSRCa = kiCa.*kCaSR;
      RI = 1 - Ry_Ri - Ry_Ro - Ry_Rr;
      values = -kim.*Ry_Ri - kom.*Ry_Ri + Ca_j.^2.*RI.*koSRCa + Ca_j.*Ry_Ro.*kiSRCa;
  elseif value_nr == 17
      values = -koff_na.*Na_Bj + kon_na.*(Bmax_Naj - Na_Bj).*Na_j;
  elseif value_nr == 18
      values = -koff_na.*Na_Bsl + kon_na.*(Bmax_Nasl - Na_Bsl).*Na_sl;
  elseif value_nr == 19
      values = -koff_tncl.*Tn_CL + kon_tncl.*(Bmax_TnClow - Tn_CL).*Ca_i;
  elseif value_nr == 20
      values = -koff_tnchca.*Tn_CHc + kon_tnchca.*(Bmax_TnChigh - Tn_CHc -...
           Tn_CHm).*Ca_i;
  elseif value_nr == 21
      values = -koff_tnchmg.*Tn_CHm + Mgi.*kon_tnchmg.*(Bmax_TnChigh - Tn_CHc -...
        Tn_CHm);
  elseif value_nr == 22
      values = -koff_cam.*CaM + kon_cam.*(Bmax_CaM - CaM).*Ca_i;
  elseif value_nr == 23
      values = -koff_myoca.*Myo_c + kon_myoca.*(Bmax_myosin - Myo_c -...
        Myo_m).*Ca_i;
  elseif value_nr == 24
      values = -koff_myomg.*Myo_m + Mgi.*kon_myomg.*(Bmax_myosin - Myo_c - Myo_m);
  elseif value_nr == 25
      values = -koff_sr.*SRB + kon_sr.*(Bmax_SR - SRB).*Ca_i;
  elseif value_nr == 26
      Bmax_SLlowj = Bmax_SLlowj0.*Vmyo./Vjunc;
      values = -koff_sll.*SLL_j + kon_sll.*(-SLL_j + Bmax_SLlowj).*Ca_j;
  elseif value_nr == 27
      Bmax_SLlowsl = Bmax_SLlowsl0.*Vmyo./Vsl;
      values = -koff_sll.*SLL_sl + kon_sll.*(-SLL_sl + Bmax_SLlowsl).*Ca_sl;
  elseif value_nr == 28
      Bmax_SLhighj = Bmax_SLhighj0.*Vmyo./Vjunc;
      values = -koff_slh.*SLH_j + kon_slh.*(-SLH_j + Bmax_SLhighj).*Ca_j;
  elseif value_nr == 29
      Bmax_SLhighsl = Bmax_SLhighsl0.*Vmyo./Vsl;
      values = -koff_slh.*SLH_sl + kon_slh.*(-SLH_sl + Bmax_SLhighsl).*Ca_sl;
  elseif value_nr == 30
      Bmax_Csqn = Bmax_Csqn0.*Vmyo./Vsr;
      values = -koff_csqn.*Csqn_b + kon_csqn.*(-Csqn_b + Bmax_Csqn).*Ca_sr;
  elseif value_nr == 31
      ...
      Bmax_Csqn = Bmax_Csqn0.*Vmyo./Vsr;
      values = -J_SRCarel + koff_csqn.*Csqn_b - kon_csqn.*(-Csqn_b +...
        Bmax_Csqn).*Ca_sr - J_SRleak.*Vmyo./Vsr + J_serca;
  elseif value_nr == 32 || value_nr == 33 || value_nr == 34
      ...
      values17 = -koff_na.*Na_Bj + kon_na.*(Bmax_Naj - Na_Bj).*Na_j;
      values18 = -koff_na.*Na_Bsl + kon_na.*(Bmax_Nasl - Na_Bsl).*Na_sl;
      if value_nr == 32
          values = -values17 + J_na_juncsl.*(-Na_j + Na_sl)./Vjunc -...
            Cmem.*I_Na_tot_junc./(Frdy.*Vjunc);
      elseif value_nr == 33
          values = -values18 + J_na_juncsl.*(-Na_sl + Na_j)./Vsl +...
          J_na_slmyo.*(-Na_sl + Na_i)./Vsl - Cmem.*I_Na_tot_sl./(Frdy.*Vsl);
      else
          values = J_na_slmyo.*(-Na_i + Na_sl)./Vmyo;
      end
  elseif value_nr == 35
      values = 0*V_m;
  elseif value_nr == 36
      ...
      J_SRCarel = ks.*(-Ca_j + Ca_sr).*Ry_Ro;
      J_SRleak = 5.348e-06.*Ca_sr - 5.348e-06.*Ca_j;
      values = -J_CaB_junction + J_ca_juncsl.*(-Ca_j + Ca_sl)./Vjunc +...
  ...
  elseif value_nr == 37
      ...
      values = -J_CaB_sl + J_ca_juncsl.*(-Ca_sl + Ca_j)./Vsl +...
  ...
  elseif value_nr == 38
      ...
      J_serca = Vmax_SRCaP.*Q10SRCaP.^Qpow.*((Ca_i./Kmf).^hillSRCaP -...
        (Ca_sr./Kmr).^hillSRCaP)./(1 + (Ca_i./Kmf).^hillSRCaP + (Ca_sr./Kmr).^hillSRCaP);
      values = -J_CaB_cytosol + J_ca_slmyo.*(-Ca_i + Ca_sl)./Vmyo -...
         J_serca.*Vsr./Vmyo;
  elseif value_nr == 39
      I_Na_junc = Fjunc.*GNa.*m.^3.*(-ena_junc + V_m).*h.*j;
      I_Na_sl = GNa.*m.^3.*(-ena_sl + V_m).*Fsl.*h.*j;
      ...
      kiss = aki./(aki + bki);
      I_ki = 0.150616019019.*sqrt(Ko).*(-ek + V_m).*kiss;
